...
#    dur = int(info.get_RecordingDuration().get_TotalSeconds())
    ...
    for cnk in tqdm(range(numReading), desc='Export & downsampling Progress'):
        ...
        for i in tqdm(ind_rec,desc='Extracting individual channel'):
            ext = np.fromiter(tmp[0][int(i)], int) # here values are converted in voltage
            raw[:,count] = ext[:]
            count +=1
...
@click.command()
@click.option('--folder','-f',prompt=True, help="Enter the full path of the folder containing the *.brw files and xx_exportCh.brw files: ")
def main(folder):
    fileCount = 1
    for filename in os.listdir(folder):
        ...
        if filename.split('_')[-1] == 'exportCh.brw':
            ...
            # print(filename, chfileName,recfileName)
