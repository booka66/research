...
from gui import DataProcessingGUI as gui
...
class ChannelExtractor:
    ...
    def process_data_callback(self, event, data):
        if event == "file_selected":
            self.file_path = data
            self.h5 = h5py.File(file_path, "r")
            OGsamplingRate = h5["/3BRecInfo/3BRecVars/SamplingRate"][0]
            recLength = h5["/3BRecInfo/3BRecVars/NRecFrames"][0] / OGsamplingRate
            h5.close()
            gui.update_labels(OGsamplingRate, recLength)
        elif event == "process_data":
            file_path = data["file_path"]
            chInt = string2TupList(data["channel_input"])
            channelVerifyTup = string2TupList(data["channel_verify"])

            if channelVerifyTup != chInt:
                gui.show_result(
                    "Channels do not match. Please verify channel indices and try again."
                )
                return

            downsampleQ = data["downsample_input"]
            if downsampleQ == "n":
                downsampleFreq = int(OGsamplingRate)
            else:
                downsampleFreq = int(downsampleQ)

            recorderName = data["recorder_name"]
            paradigm = data["paradigm"]
            brainRegion = data["brain_region"]

            # Perform the processing logic here
            gui.show_result("Data processed successfully!")
    def initialize_channel_list(self):
        ...
        clr.AddReference(
            os.path.join(
                "C:\\Program Files\\3Brain\\BrainWave 5", "3Brain.BrainWave.IO.dll"
            )
        )
...
def string2TupList(s):
    return [tuple(map(int, pair.strip("()").split(","))) for pair in s.split(",")]
...
def main(fpath):
    ...
    filematch = True
    chfileName = fpath
    chfileInfo = get_chfile_properties(fpath)
    ...
    fileInfo["numRecElectrodes"] = len(fileInfo["recElectrodeList"])
    fileInfo["Ver"] = 1  # Artifact var
    fileInfo["Typ"] = 1  # Artifact var
    fileInfo["start"] = (
        0  ##I'm going to put 0 for the start of the recording; This is adjustable
    )
    fileInfo["end"] = recLength  # uses the end of the recording as end cutoff.
    recfilePath = fpath
    recfileName = fpath
    parameters = get_recFile_properties(
        recfilePath, 1
